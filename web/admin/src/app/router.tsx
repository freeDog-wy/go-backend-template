...
import { ArticlesPage, CategoriesPage, LocalesPage, LoginPage, TagsPage } from "../features/pages";
...
const articleEditor = async () => ({ Component: (await import("../features/articles/ArticleEditorPage")).ArticleEditorPage });
const router = createHashRouter([{ path: "/login", element: <LoginRoute /> }, { element: <Protected />, children: [{ element: <AdminLayout />, children: [{ index: true, element: <Navigate to="/articles" replace /> }, { path: "articles", element: <ArticlesPage /> }, { path: "articles/new", lazy: articleEditor }, { path: "articles/:id/:locale", lazy: articleEditor }, { path: "categories", element: <CategoriesPage /> }, { path: "tags", element: <TagsPage /> }, { path: "locales", element: <LocalesPage /> }] }] }, { path: "*", element: <Navigate to="/articles" replace /> }]);
