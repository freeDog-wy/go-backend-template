...
func newMCPServiceTokenHandler(ctx context.Context, cfg *config.Config, infra *serverInfrastructure, repos *serverRepositories, auditRecorder platformAudit.Recorder) (*hdlServiceToken.Handler, error) {
	if !cfg.MCP.Enabled {
		return nil, nil
	}
