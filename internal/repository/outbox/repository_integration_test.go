...
func TestRepositoryIntegrationPublishLifecycle(t *testing.T) {
	...
	eventOne, _ := domainOutbox.NewEvent(eventOneName, `{"id":1}`, "trace-1", "")
	eventTwo, _ := domainOutbox.NewEvent(eventTwoName, `{"id":2}`, "trace-2", "")
	if err := repo.Create(context.Background(), eventOne, eventTwo); err != nil {
		t.Fatalf("Create() error = %v", err)
	}
	t.Cleanup(func() {
		_ = db.Where("event_name IN ?", []string{eventOneName, eventTwoName}).Delete(&modelOutbox.Event{}).Error
	})
