...
func (r *Repository) Create(ctx context.Context, a *model.Asset) error {
	return repositorytx.DB(ctx, r.db).Create(a).Error
}
func (r *Repository) SetUploadExpiresAt(ctx context.Context, id uint, expiresAt time.Time) error {
	res := repositorytx.DB(ctx, r.db).Model(&model.Asset{}).Where("id = ? AND status = 'pending'", id).Update("upload_expires_at", expiresAt)
	if res.Error != nil {
		return res.Error
	}
...
func (r *Repository) Find(ctx context.Context, id uint) (*model.Asset, error) {
	var a model.Asset
	if err := repositorytx.DB(ctx, r.db).Where("deleted_at IS NULL").First(&a, id).Error; err != nil {
...
func (r *Repository) MarkReady(ctx context.Context, id uint, mime string, size int64, width, height int, now time.Time) error {
	res := repositorytx.DB(ctx, r.db).Model(&model.Asset{}).Where("id = ? AND status = 'pending' AND (upload_expires_at IS NULL OR upload_expires_at > ?)", id, now).Updates(map[string]any{"status": "ready", "mime_type": mime, "size_bytes": size, "width": width, "height": height})
	if res.Error != nil {
		return res.Error
	}
	if res.RowsAffected == 0 {
...
func (r *Repository) MarkExpired(ctx context.Context, id uint, now time.Time) error {
	return repositorytx.DB(ctx, r.db).Model(&model.Asset{}).Where("id = ? AND status = 'pending' AND upload_expires_at <= ?", id, now).Update("status", "expired").Error
}
...
func (r *Repository) MarkDeleted(ctx context.Context, id uint, now time.Time) error {
	res := repositorytx.DB(ctx, r.db).Model(&model.Asset{}).Where("id = ? AND status IN ('expired', 'failed')", id).Updates(map[string]any{"status": "deleted", "deleted_at": now, "cleanup_last_error": "", "cleanup_claimed_at": nil})
	if res.Error != nil {
		return res.Error
	}
	if res.RowsAffected == 0 {
...
func (r *Repository) RecordCleanupFailure(ctx context.Context, id uint, message string) error {
	return repositorytx.DB(ctx, r.db).Model(&model.Asset{}).Where("id = ? AND status IN ('expired', 'failed')", id).Updates(map[string]any{"cleanup_attempts": gorm.Expr("cleanup_attempts + 1"), "cleanup_last_error": message, "cleanup_claimed_at": nil}).Error
}
func (r *Repository) MarkFailed(ctx context.Context, id uint, reason string) error {
	res := repositorytx.DB(ctx, r.db).Model(&model.Asset{}).Where("id = ? AND status = 'pending'", id).Updates(map[string]any{"status": "failed", "failure_reason": reason})
	if res.Error != nil {
		return res.Error
	}
	if res.RowsAffected == 0 {
...
func (r *Repository) List(ctx context.Context, limit, offset int) ([]model.Asset, int64, error) {
	var total int64
	db := repositorytx.DB(ctx, r.db).Model(&model.Asset{}).Where("deleted_at IS NULL")
...
func (r *Repository) ListReadyPublic(ctx context.Context, locale string, ids []uint) ([]domainMedia.PublicAsset, error) {
	...
	var rows []row
	err := repositorytx.DB(ctx, r.db).Table("media_assets").
...
func (r *Repository) UpsertTranslation(ctx context.Context, t *model.Translation) error {
	return repositorytx.DB(ctx, r.db).Clauses(clause.OnConflict{Columns: []clause.Column{{Name: "media_id"}, {Name: "locale"}}, DoUpdates: clause.Assignments(map[string]any{"alt_text": t.AltText, "title": t.Title, "updated_at": time.Now()})}).Create(t).Error
}
