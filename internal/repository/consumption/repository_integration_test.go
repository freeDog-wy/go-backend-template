...
func TestRepositoryIntegrationBegin(t *testing.T) {
	...
	t.Run("acquires new message and creates processing record", func(t *testing.T) {
		...
		result, err := repo.Begin(context.Background(), domainConsumption.BeginCommand{
		...
		if result.Decision != domainConsumption.BeginDecisionAcquired {
			t.Fatalf("decision = %q, want %q", result.Decision, domainConsumption.BeginDecisionAcquired)
		}
		...
		if record.Status != string(domainConsumption.StatusProcessing) {
			t.Fatalf("status = %q, want %q", record.Status, domainConsumption.StatusProcessing)
		}
	...
	t.Run("returns locked when processing record lock is still active", func(t *testing.T) {
		...
		attemptedAt := time.Now().UTC().Truncate(time.Second)
		lockedUntil := attemptedAt.Add(5 * time.Minute)
		seedConsumptionRecord(t, db, &modelConsumption.Record{
			ConsumerGroup: consumerGroup,
			MessageKey:    messageKey,
			EventName:     "user.registered",
			TraceID:       "trace-1",
		...
		if result.Decision != domainConsumption.BeginDecisionLocked {
			t.Fatalf("decision = %q, want %q", result.Decision, domainConsumption.BeginDecisionLocked)
		}
	...
	t.Run("returns done when message is already done", func(t *testing.T) {
		...
		if result.Decision != domainConsumption.BeginDecisionDone {
			t.Fatalf("decision = %q, want %q", result.Decision, domainConsumption.BeginDecisionDone)
		}
		if result.AttemptCount != 2 {
			t.Fatalf("attempt count = %d, want 2", result.AttemptCount)
		}
...
func TestRepositoryIntegrationStatusUpdates(t *testing.T) {
	...
	t.Run("MarkDone updates status and processed metadata", func(t *testing.T) {
		...
		record := findConsumptionRecord(t, db, consumerGroup, messageKey)
		if record.Status != string(domainConsumption.StatusDone) {
			t.Fatalf("status = %q, want %q", record.Status, domainConsumption.StatusDone)
		}
	...
	t.Run("MarkFailed updates failure state", func(t *testing.T) {
		...
		record := findConsumptionRecord(t, db, consumerGroup, messageKey)
		if record.Status != string(domainConsumption.StatusFailed) {
			t.Fatalf("status = %q, want %q", record.Status, domainConsumption.StatusFailed)
		}
		if record.LockedUntil != nil {
			t.Fatalf("locked until = %v, want nil", record.LockedUntil)
		}
	...
	t.Run("MarkDead updates dead state", func(t *testing.T) {
		...
		record := findConsumptionRecord(t, db, consumerGroup, messageKey)
		if record.Status != string(domainConsumption.StatusDead) {
			t.Fatalf("status = %q, want %q", record.Status, domainConsumption.StatusDead)
		}
		if record.LockedUntil != nil {
			t.Fatalf("locked until = %v, want nil", record.LockedUntil)
		}
...
func TestRepositoryIntegrationValidation(t *testing.T) {
	...
	_, err := repo.Begin(context.Background(), domainConsumption.BeginCommand{})
	if !errors.Is(err, domainConsumption.ErrInvalidRecord) {
		t.Fatalf("Begin() error = %v, want %v", err, domainConsumption.ErrInvalidRecord)
	}

	if err := repo.MarkDone(context.Background(), "", "message-1", time.Now()); !errors.Is(err, domainConsumption.ErrInvalidRecord) {
		t.Fatalf("MarkDone() error = %v, want %v", err, domainConsumption.ErrInvalidRecord)
	}
	if err := repo.MarkFailed(context.Background(), "group", "", "failed", time.Now()); !errors.Is(err, domainConsumption.ErrInvalidRecord) {
		t.Fatalf("MarkFailed() error = %v, want %v", err, domainConsumption.ErrInvalidRecord)
	}
	if err := repo.MarkDead(context.Background(), "group", "", "dead", time.Now()); !errors.Is(err, domainConsumption.ErrInvalidRecord) {
		t.Fatalf("MarkDead() error = %v, want %v", err, domainConsumption.ErrInvalidRecord)
	}
}
...
func seedConsumptionRecord(t *testing.T, db *gorm.DB, record *modelConsumption.Record) {
	t.Helper()
	...
	t.Cleanup(func() {
		_ = db.Where("consumer_group = ? AND message_key = ?", record.ConsumerGroup, record.MessageKey).
			Delete(&modelConsumption.Record{}).Error
	})
}

func findConsumptionRecord(t *testing.T, db *gorm.DB, consumerGroup, messageKey string) modelConsumption.Record {
	t.Helper()
