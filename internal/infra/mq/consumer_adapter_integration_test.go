...
func TestConsumerAdapterIntegrationConsumesAndMarksDone(t *testing.T) {
	...
	publisher := pkgkafka.NewPublisher(broker.Brokers, topic, pkgkafka.WriterConfig{ClientID: "integration-producer"})
	if err := publisher.Publish(context.Background(), pkgkafka.Message{
		Key:     []byte(messageKey),
		Value:   []byte(`{"id":42}`),
		Headers: []pkgkafka.Header{{Key: "event", Value: []byte("integration.event")}},
	}); err != nil {
