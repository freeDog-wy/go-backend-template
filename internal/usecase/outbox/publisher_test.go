...
func TestPublishPending(t *testing.T) {
	...
	t.Run("publishes all events and marks them published", func(t *testing.T) {
		...
		repo := &stubOutboxRepository{
			events: []*domainOutbox.Event{event1, event2, event3},
		}
		publisher := &stubOutboxPublisher{}
	...
	t.Run("returns publish error when first publish fails and marks no ids", func(t *testing.T) {
		...
		repo := &stubOutboxRepository{
			events: []*domainOutbox.Event{event1, event2},
		}
		publisher := &stubOutboxPublisher{
			failAtCall: 1,
			publishErr: errors.New("kafka unavailable"),
...
func (r *stubOutboxRepository) Create(context.Context, ...*domainOutbox.Event) error {
	return nil
}

func (r *stubOutboxRepository) ListUnpublished(_ context.Context, limit int) ([]*domainOutbox.Event, error) {
	r.listLimit = limit
	if r.listErr != nil {
...
func newTestOutboxEvent(id uint, eventName, payload, traceID, traceContext string) *domainOutbox.Event {
	return domainOutbox.ReconstituteEvent(id, eventName, payload, traceID, traceContext, nil, time.Now())
}
...
func assertPublishedCall(t *testing.T, got publishCall, event *domainOutbox.Event) {
	t.Helper()
